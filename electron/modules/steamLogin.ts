...
/**
 * Open modal Steam login window with local HTTP callback
 */
export function openSteamLoginWindow(mainWindow: BrowserWindow): Promise<boolean> {
    ...
    const server = http.createServer(async (req: any, res: any) => {
      try {
        ...
        if (parsed.pathname === '/auth') {
          ...
          console.log('Local callback hit. claimed:', claimedStr, 'steamId64:', steamId64);
          ...
          if (steamId64) {
            ...
            if (!apiKey) {
              ...
              const profile = await fetchSteamProfile(apiKey, steamId64);
              if (profile) {
                upsertUser(profile);
                mainWindow.webContents.send('steam-user-updated', { success: true, steamId64 });
              } else {
                mainWindow.webContents.send('steam-user-updated', {
                  success: false,
            ...
            console.error('Could not extract steamId64 from claimed:', claimedStr);
            mainWindow.webContents.send('steam-user-updated', {
              success: false,
